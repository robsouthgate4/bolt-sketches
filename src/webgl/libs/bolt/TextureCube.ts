...
import { CLAMP_TO_EDGE, LINEAR, RGBA, TEXTURE_CUBE_MAP, TEXTURE_CUBE_MAP_NEGATIVE_X, TEXTURE_CUBE_MAP_NEGATIVE_Y, TEXTURE_CUBE_MAP_NEGATIVE_Z, TEXTURE_CUBE_MAP_POSITIVE_X, TEXTURE_CUBE_MAP_POSITIVE_Y, TEXTURE_CUBE_MAP_POSITIVE_Z, UNSIGNED_BYTE, } from "./Constants";
...
export default class TextureCube extends Texture {
	...
	_init() {
		...
		for ( let i = 0; i < 6; i ++ ) {
			...
			this.gl.texImage2D( this.gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, this._internalFormat, this._width, this._height, 0, this._format, this._pixelType, null );
	...
	setFromData( data: Float32Array | Uint16Array | Uint8Array, width: number, height: number ) {
		...
		for ( let i = 0; i < 6; i ++ ) {
			...
			this.gl.texImage2D( this.gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, this._internalFormat, width, height, 0, this._format, this._type, data );
