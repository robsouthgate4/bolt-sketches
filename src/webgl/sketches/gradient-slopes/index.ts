...
export default class extends Base {
  ...
  peakScale = new EaseVec3(
    config.peakScale.value.x,
    config.peakScale.value.y,
    config.peakScale.value.z,
    0.1
  );
  colorNoiseScale = new EaseVec3(
    config.colorNoiseScale.value.x,
    config.colorNoiseScale.value.y,
    config.colorNoiseScale.value.z,
    0.1
  );
  ...
  async initSketch() {
    ...
    const drawState1 = new DrawState(this.bolt)
      ...
      .uniformVector3(
        "color1",
        vec3.fromValues(color1[0], color1[1], color1[2])
      )
      .uniformVector3(
        "color2",
        vec3.fromValues(this.color2.x, this.color2.y, this.color2.z)
      )
      .uniformVector3(
        "color3",
        vec3.fromValues(this.color3.x, this.color3.y, this.color3.z)
      )
    ...
    const drawState2 = new DrawState(this.bolt)
      ...
      .uniformVector3(
        "color1",
        vec3.fromValues(this.color1.x, this.color1.y, this.color1.z)
      )
      .uniformVector3(
        "color2",
        vec3.fromValues(this.color2.x, this.color2.y, this.color2.z)
  ...
  update(elapsed: number, delta: number) {
    ...
    this.drawCalls.forEach((drawCall: DrawCall) => {
      ...
      drawState
        ...
        .uniformVector3(
          "peakScale",
          vec3.fromValues(this.peakScale.x, this.peakScale.y, this.peakScale.z)
        )
        .uniformVector3(
          "colorNoiseScale",
          vec3.fromValues(
            this.colorNoiseScale.x,
            this.colorNoiseScale.y,
            this.colorNoiseScale.z
          )
        )
        .uniformVector2(
          "resolution",
          vec2.fromValues(ctx.canvas.width, ctx.canvas.height)
        )
        .uniformVector3(
          "color2",
          vec3.fromValues(this.color2.x, this.color2.y, this.color2.z)
