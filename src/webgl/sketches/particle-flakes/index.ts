...
export default class extends Base {
  ...
  constructor() {
    ...
    this.repellorDebug = new DrawSet(
      new Mesh(new Sphere()),
      new Program(normalVertexShader, normalFragmentShader)
    );
  ...
  initListeners() {
    this.eventListeners.listen(GL_TOUCH_MOVE_TOPIC, (e: any) => {
      ...
      this.ray = this.raycaster.generateRayFromCamera(
        normalized.x,
        normalized.y,
        this.camera
      );
  ...
  initScene() {
    ...
    mat4.multiply(
      ...
      this.shadowLight.projection,
      this.shadowLight.view
    );
  ...
  async initSketch() {
    ...
    if (localStorage.getItem("particleCount") !== null) {
      this.instanceCount = parseInt(
        localStorage.getItem("particleCount") as string
      );
    ...
    this.particleProgram = new Program(
      particlesVertexInstanced,
      particlesFragmentInstanced
    );
    ...
    this.simulationProgram.setFloat(
      "particleLifeRate",
      this.config.particleLifeRate
    );
    this.simulationProgram.setFloat(
      "particleLifeTime",
    ...
    await gltfLoader.load(
      "/static/models/gltf/examples/disk-particle/scene.glb"
    );
  ...
  initGUI() {
    ...
    folder
      .add(this.config, "particleCount", [5000, 10000, 20000, 30000])
    ...
    folder
      .add(this.config, "particleScale", 0.1, 1)
      .step(0.01)
      .onChange((value: number) => {
    ...
    folder
      .add(this.config, "particleSpeed", 0.1, 2)
      .step(0.1)
      .onChange((value: number) => {
        this.simulationProgram.activate();
    ...
    folder
      .add(this.config, "particleLifeRate", 0.01, 0.08)
      .step(0.01)
      .onChange((value: number) => {
        this.simulationProgram.activate();
    ...
    folder
      .add(this.config, "curlStrength", 1, 5)
      .step(0.1)
      .onChange((value: number) => {
        this.simulationProgram.activate();
    ...
    folder
      .add(this.config, "shadowStrength", 0.1, 1)
      .step(0.1)
      .onChange((value: number) => {
    ...
    folder
      .add(this.config, "colorMode", ["light", "dark"])
  ...
  update(elapsed: number, delta: number) {
    ...
    vec3.set(
      this.repellorPosition,
      this.repellorTarget.x,
      this.repellorTarget.y,
      this.repellorTarget.z
    );
    ...
    this.simulationProgram.setVector3(
      "repellorPosition",
      this.repellorPosition
    );
    ...
    this.particleDrawState
      ...
      .clear(
        bgLight[0] * (1 - this.colorEase.value) +
          bgDark[0] * this.colorEase.value,
        bgLight[1] * (1 - this.colorEase.value) +
          bgDark[1] * this.colorEase.value,
        bgLight[2] * (1 - this.colorEase.value) +
          bgDark[2] * this.colorEase.value,
        bgLight[3] * (1 - this.colorEase.value) +
          bgDark[3] * this.colorEase.value
      )
      .draw();
